...
const LeaveRequests = () => {
  ...
  const handleAddLeave = async () => {
    ...
    try {
      ...
      setNewLeave({ leaveType: "", startDate: "", endDate: "", reason: "", isHalfDay: false });
      setShowAddDialog(false);
      await fetchLeaves();
      await fetchCurrentEmployee();
      toast.success("Leave request submitted successfully!");
              ...
              <TableRow key={request.id}>
                <TableCell>
                  <div className="flex items-center space-x-3">
                    <Avatar className="w-8 h-8">
                      <AvatarImage
                        src={request.profileImage || request.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(request.employeeName)}&background=0D8ABC&color=fff`}
                        alt={request.employeeName}
                      />
                      <AvatarFallback>
                ...
                <TableCell>{request.duration} day(s)</TableCell>
                <TableCell>{request.leaveBalance != null ? `${request.leaveBalance} day(s)` : "—"}</TableCell>
                <TableCell>{getStatusBadge(request.status)}</TableCell>
