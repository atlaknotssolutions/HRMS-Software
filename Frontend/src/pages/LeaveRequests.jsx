...
//       return {
//         startDate: formatDate(today),
//         endDate: formatDate(addDays(today, 5)), // Changed to 5 days
//       };
//     }
//     if (type === "Maternity Leave") {
//       return {
...
//   // Auto update End Date when Start Date changes for Maternity & Paternity
...
//     } else {
//       return; // Don't auto-update for other leave types
//     }
...
//   const countedLeaveRequests = leaveRequests.filter(
//     (r) => !["Maternity Leave", "Paternity Leave"].includes(r.leaveType),
//   );
//   const pendingRequests = countedLeaveRequests.filter(
//     (r) => r.status === "pending",
//   ).length;
//   const approvedRequests = countedLeaveRequests.filter(
//     (r) => r.status === "approved",
//   ).length;
//   const totalRequests = countedLeaveRequests.length;
...
//                       setNewLeave((prev) => ({ ...prev, startDate: newStart }));
...
const LeaveRequests = () => {
  ...
  // Stats (excluding Maternity & Paternity for normal counts)
  ...
  const handleAddLeave = async () => {
    ...
    // Block submission only for normal leaves if balance is insufficient
              ...
              <div className="space-y-4">
                <div>
                  ...
                  <Select
                    ...
                    onValueChange={(value) => {
                      const autoDates = getAutoDatesForLeaveType(value);
                      setNewLeave({ ...newLeave, leaveType: value, ...autoDates });
                    }}
                  >
                    <SelectTrigger>
                      <SelectValue placeholder="Select leave type" />
                ...
                {currentLeaveBalance != null && (
                  <div className="p-4 bg-slate-50 dark:bg-slate-950 border border-slate-200 dark:border-slate-800 rounded-lg">
                    ...
                    {!isSpecialLeave && projectedRemaining !== null && (
                      <p className={`mt-2 text-sm font-medium ${projectedRemaining < 0 ? "text-destructive" : "text-foreground"}`}>
                        {projectedRemaining < 0
                          ? `Insufficient balance: ${formatDays(projectedRemaining)} day(s) left`
                          : `After this request: ${formatDays(projectedRemaining)} day(s)`}
                      </p>
